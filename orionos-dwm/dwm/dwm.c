...
/**
 * custom DWM (Dynamic Window Manager) for orionos
 *
 * Xathrya    (2017) <xathrya@orionos-linux.org>
 * Randalltux (2016) <rndtx@orionos-linux.org>
 */
...
enum { SchemeNorm, SchemeSel, SchemeTag, SchemeLayout,
       SchemeTitle, SchemeStText, SchemeStIcon, SchemeLast };      /* color schemes */
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, iscentered, needresize;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
static void togglesecondbar(const Arg *arg);
...
#include "patches/layouts/column.c"
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, &scheme[(m->tagset[m->seltags] & 1 << i) ? SchemeSel : (urg & 1 << i ? SchemeTag : SchemeNorm)]);
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		w = drw_get_width(drw, SchemeLast, stext);
		x = m->ww - w;
		if (x < xx) {
			x = xx;
			w = m->ww - xx;
		}
		drw_colored_text(drw, scheme, SchemeLast, x, 0, w, bh, stext);
	} else
		x = m->ww;
	if ((w = x - xx) > bh) {
		x = xx;
